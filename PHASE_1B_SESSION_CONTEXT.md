...
### 2.7 Capacity profiling module location

- Step 3 capacity profiling follows test-first ml_utils development
- Tests: tests/test_phase1b_capacity_profile.py
- Core helpers: ml_utils/profiling.py
- Optional later wrapper: scripts/phase1b_capacity_profile.py
- Wrapper 只负责 data loading、CSV output、printed verdicts，必须 import ml_utils.profiling helpers，不许复制 core profiling logic
...
Step 3:  ml_utils/profiling.py capacity profiling（下一步）
...
ml_utils/seed.py
ml_utils/metrics.py
ml_utils/profiling.py（Step 3 待建）
ml_utils/dataset.py
