...
def test_protocol_metadata_locks_baseline_fixed_5bps_route():
...
def test_default_feature_set_resolves_to_baseline_v1():
    args = Namespace(feature_set=None)

    assert runner.resolve_feature_set(args, "smoke") == "baseline_v1"
    assert runner.resolve_feature_set(args, "full") == "baseline_v1"


def test_baseline_v1_feature_set_excludes_raw_price_volume_and_raw_macd():
    df = _technical_raw_frame(rows=80)

    feature_df = runner.add_feature_set(df, "baseline_v1")
    ...
    assert tuple(runner.FEATURE_SETS["baseline_v1"]) == runner.BASELINE_V1_FEATURES
    assert set(runner.BASELINE_V1_FEATURES) == expected_features
    assert set(runner.BASELINE_V1_FEATURES).isdisjoint(forbidden_features)
    assert expected_features.issubset(feature_df.columns)
    assert np.isfinite(feature_df.loc[:, runner.BASELINE_V1_FEATURES].to_numpy()).all()


def test_baseline_v1_exact_values_for_hand_checked_row():
    df = _technical_raw_frame(rows=80)
...
def test_baseline_v1_resets_lagged_features_at_trading_day_boundary():
...
def test_sklearn_validation_only_cli_marks_metadata_and_result_scope(
    ...
    metadata_path = next((tmp_path / "out").rglob("metadata.json"))
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    assert metadata["feature_set_id"] == "baseline_v1"
...
def test_prepare_data_validation_only_does_not_call_three_way_time_split(
    ...
    prepared = runner.prepare_data(
        ...
        feature_cols=list(runner.BASELINE_V1_FEATURES),
        candidate=_no_trade_candidate(),
        max_rows_per_ticker=None,
        calendar_split=None,
        shuffle_train_labels=False,
...
def test_torch_validation_only_ms_dlinear_tcn_cli_uses_safe_branch(
    ...
    assert observed["feature_cols"] == list(runner.FEATURE_SETS["baseline_v1"])
    assert observed["metadata"]["models"] == ["ms_dlinear_tcn"]
    assert observed["metadata"]["model_family"] == "torch"
    _assert_baseline_validation_only_metadata(observed["metadata"])
    results = pd.read_csv(next((tmp_path / "out").rglob("results.csv")))
...
@pytest.mark.parametrize(
    ...
    [
        ({"feature_set": "ohlcv_only_v1"}, "baseline_v1"),
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "lightgbm.*--feature-set baseline_v1",
        ),
        (
            [
                "--feature-set",
...
def _assert_baseline_validation_only_metadata(metadata):
    assert metadata["feature_set_id"] == "baseline_v1"
    assert metadata["label_mode"] == "no_trade_band"
    assert metadata["threshold_bps"] == 5.0
    assert metadata["report_scope"] == "validation_only"
...
def _no_trade_metadata():
    ...
    metadata.update(
        {
            "feature_set_id": "baseline_v1",
            "feature_columns": list(runner.BASELINE_V1_FEATURES),
            "label_mode": "no_trade_band",
