...
- Create or review this single protocol-lock document.
...
packet is not sufficient.

## Feature Availability Table

| Feature | Current implementation basis | Decision-time availability | Warmup / grouping | Lock decision |
| --- | --- | --- | --- | --- |
...
- Forbidden: fitting on validation/test, fitting before chronological splits,
  per-full-sample normalization, or changing scaler type without a new protocol
  lock.
...
- Validation-selected thresholds are allowed only in a separate approved task,
  must use train/validation only, and must be locked before any test readout.
...
MS-DLinear+TCN decision: blocked.
...
- Confirm this lock packet is the active route.
...
- Prefer `sklearn_logreg` validation-only reporting as a cheap leak/check smoke,
  because LightGBM and combined MS-DLinear+TCN are blocked.
...
Step 3: Implementation unblock review.

- If tiny validation passes, choose one blocker to unblock:
  LightGBM adapter or combined MS-DLinear+TCN model spec.
- Add tests before training.
- Keep paper/evidence-matrix updates blocked until real validation artifacts
