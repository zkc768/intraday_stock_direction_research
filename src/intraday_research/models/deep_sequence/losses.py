...
def _is_float_scalar(value: object) -> bool:
    ...
    return arr.ndim == 0 and np.issubdtype(arr.dtype, np.floating)


def _is_int_scalar(value: object) -> bool:
    if isinstance(value, (bool, np.bool_)):
        return False
    arr = np.asarray(value)
...
def _validate_prior(
    train_class_prior: tuple[float, float], *, name: str = "train_class_prior"
) -> tuple[float, float]:
    ...
    prior = tuple(float(p) for p in train_class_prior if _is_float_scalar(p))
    if len(prior) != 2 or not all(np.isfinite(p) and 0.0 < p < 1.0 for p in prior):
        raise ValueError(f"{name} entries must be float scalars in (0, 1); got {train_class_prior!r}")
    if abs(sum(prior) - 1.0) > 1e-6:
        raise ValueError(
            f"{name} must sum to 1.0; got {train_class_prior!r} (sum={sum(prior)})"
        )
    return prior


def _validate_samples_per_class(samples_per_class: tuple[int, int]) -> tuple[int, int]:
    ...
    counts = tuple(int(n_c) for n_c in samples_per_class if _is_int_scalar(n_c))
    if len(counts) != 2 or not all(n_c >= 1 for n_c in counts):
        raise ValueError(f"samples_per_class entries must be int scalars >= 1; got {samples_per_class!r}")
    return counts
...
def weighted_cross_entropy_train_prior_loss(
    ...
    *,
    ...
    """
    prior = _validate_prior(train_class_prior)
    weight = np.array(
        [1.0 / prior[0], 1.0 / prior[1]], dtype=np.float64
    )
...
def focal_loss(
    ...
    *,
    ...
    """
    log_arr, t = _validate_logits_targets(logits, targets)
    if not _is_float_scalar(gamma):
        raise ValueError(f"gamma must be a non-negative float; got {gamma!r}")
    gamma_value = float(gamma)
    if not np.isfinite(gamma_value) or gamma_value < 0.0:
        raise ValueError(f"gamma must be a non-negative finite float; got {gamma!r}")
    alpha_value = None
    if alpha is not None:
        if not _is_float_scalar(alpha):
            raise ValueError(f"alpha must be a float in [0, 1] or None; got {alpha!r}")
        alpha_value = float(alpha)
    if alpha_value is not None and (not np.isfinite(alpha_value) or not (0.0 <= alpha_value <= 1.0)):
        raise ValueError(f"alpha must be a float in [0, 1] or None; got {alpha!r}")
    log_p_t = _log_softmax(log_arr)[np.arange(t.shape[0]), t]
    p_t = np.exp(log_p_t)
    fl = (1.0 - p_t) ** gamma_value * (-log_p_t)
    if alpha_value is not None:
        fl = np.where(t == 1, alpha_value, 1.0 - alpha_value) * fl
...
def class_balanced_loss_effective_number(
    ...
    *,
    ...
    """
    log_arr, t = _validate_logits_targets(logits, targets)
    counts_tuple = _validate_samples_per_class(samples_per_class)
    if not _is_float_scalar(beta):
        raise ValueError(f"beta must be a float in (0, 1); got {beta!r}")
    beta_value = float(beta)
    if not np.isfinite(beta_value) or not (0.0 < beta_value < 1.0):
        raise ValueError(f"beta must be a finite float in (0, 1); got {beta!r}")
    ...
    effective = -np.expm1(counts * np.log1p(beta_value - 1.0)) / (1.0 - beta_value)
...
def balanced_softmax_loss(
    ...
    *,
    ...
    after row-centering the logits — ``CE(centered_logits + log(prior), targets)``.
    ...
    log_arr, t = _validate_logits_targets(logits, targets)
    prior = _validate_prior(train_class_prior)
    log_prior = np.log(np.array(prior, dtype=np.float64))
