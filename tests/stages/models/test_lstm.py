"""Tests for the N08 #5D-4 shallow LSTM classifier body (CPU PyTorch).

Mirrors test_gru.py: LSTM is a near-copy of GRU on the shared
``_SequenceTorchClassifier`` base, with ``nn.LSTM`` and a frozen ``proj_size=0``.
"""
...
def _make_sequence_signal(n: int = 200, length: int = 20, seed: int = 1):
    ...
    every window, so only a model that reads the temporal shape (the LSTM output
...
def _fit(**kw):
    ...
    return ShallowLSTMClassifier(**kw)


# ---- 1. Protocol -------------------------------------------------------

def test_is_sequence_classifier():
    assert isinstance(ShallowLSTMClassifier(), SequenceClassifier)
...
def test_causality_no_future_leak(head, num_layers):
    ...
    # Causal: LSTM output rows at time <= t depend only on inputs <= t (identical)
    # -> bit-identical. A bidirectional LSTM would mix future bars into earlier
    # rows and fail this.
...
def test_dropout_num_layers_one_emits_no_dropout_warning():
    # dropout=0.0 to nn.LSTM + a pooled nn.Dropout must suppress torch's
    # "dropout expects num_layers>1" UserWarning.
    X, y = _make_xy(n=20, seed=2)
...
@pytest.mark.parametrize("num_layers", [1, 2])
def test_dropout_axis_is_pooled_not_lstm_internal(num_layers):
    X, y = _make_xy(n=20, seed=2)
    clf = _fit(dropout=0.20, num_layers=num_layers).fit(X, y)
    # The dropout axis is realized as a pooled-feature nn.Dropout, never nn.LSTM's
    # between-layer dropout; proj_size stays 0 (no LSTMP projection) — Codex P3.
    assert clf._model.lstm.dropout == 0.0
    assert clf._model.lstm.proj_size == 0
